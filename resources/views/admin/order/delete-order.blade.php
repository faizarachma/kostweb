<button type="button" class="delete-button bg-red-500 text-white p-2 rounded-full hover:bg-red-600"
    data-id="{{ $pesanan->id }}">
...
<div id="deleteModal-{{ $pesanan->id }}"
    ...
    <div class="bg-white p-6 rounded-lg shadow-md w-1/3">
        ...
        <div class="flex justify-end space-x-4 mt-4">
            <button type="button" onclick="closeDeleteModal({{ $pesanan->id }})"
                class="bg-gray-300 text-gray-700 px-4 py-2 rounded-full hover:bg-gray-400">
                Batal
            </button>
            <form action="{{ route('pemesanan.destroy', $pesanan->id) }}" method="POST">
                @csrf
                @method('DELETE')
